...
function predictToAllUsers(){
	...
	for($i=0;$i<count($p);$i++){
		$users = getUnratedItems($p[$i]);
		for($j=0;$j<count($users);$j++){			
			echo "Predict product $p[$i] to user $users[$j] on the basis of: ";print_r(predictProduct($p[$i], $users[$j]));echo "<br>";
...
function predictProduct($productID, $toUser){
	...
	foreach($data as $key=>$value){
		...
		if(PCC($rv,getRatedColumnValues($productID)) > $threshold){
...
print_r(predictToSingleUser(65));
?>